...
import { spaceInformation } from "../Fun-Artemis2/space.js";
...
`);
  ...
  const earth = new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({
      color: 0xffffff,
      map: textloader.load("../Fun-Artemis2/Earth (A).jpg"),
    }),
  );
  ...
  const moon = new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({
      color: 0xffffff,
      map: textloader.load("../Fun-Artemis2/lroc_color_2k.jpg"),
    }),
  );
  ...
  let predictionTotalDurationSec = 0;
  let executePlannedDurationSec = 0;
  let executeElapsedSec = 0;
  let isExecutingPredictionPlan = false;
  ...
  const updatePredictedTrajectory = () => {
    ...
    predictionTotalDurationSec = simulatedSteps * trajectoryPredictionTimeStep;
  ...
  const startExecuteFromCurrentPrediction = () => {
    executePlannedDurationSec = Math.max(0, predictionTotalDurationSec);
    executeElapsedSec = 0;
    isExecutingPredictionPlan = executePlannedDurationSec > 0;
  };

  const stopExecutePlan = () => {
    isExecutingPredictionPlan = false;
    executeElapsedSec = 0;
  };
  ...
  __add_nextframe_fn__(() => {
    ...
    if (isExecutingPredictionPlan) {
      executeElapsedSec += simStepSeconds;
      if (executeElapsedSec >= executePlannedDurationSec) {
        stopExecutePlan();
  ...
  __add_nextframe_fn__(() => {
    // Design mode: system paused, but prediction and guides should keep reacting.
    if (isSystemPaused) return;
  ...
  __add_nextframe_fn__((x, y, z, delta, skip) => {
    ...
    __usePanel_write__(
      7,
      `prediction duration: ${formatSimDuration(predictionTotalDurationSec)}`,
    );
    __usePanel_write__(
      8,
      isExecutingPredictionPlan
        ? `plan progress: ${formatSimDuration(executeElapsedSec)} / ${formatSimDuration(executePlannedDurationSec)}`
        : "plan progress: idle",
    );
  }, 1);
  ...
  __updateTHREEJs__only__.isSystemPaused = (yes) => {
    isSystemPaused = yes;
    if (isSystemPaused) {
      startExecuteFromCurrentPrediction();
    } else {
      stopExecutePlan();
    }
  };
...
__defineControl__("isSystemPaused", "bit", isSystemPaused, {
  label: "paused",
  eval: true,
});
